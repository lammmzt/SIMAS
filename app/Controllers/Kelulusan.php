...
//   ============================= TRANSKRIP =============================
    ...
    public function Transkrip(): string // menampilkan halaman dashboard
    { 
        $semesterModel = new semesterModel();
        $data['semester'] = $semesterModel->findAll();
        $data['title'] = 'SIMAS | Transkrip Ijazah'; // set judul halaman
        $data['active'] = 'Transkrip'; // set active menu
        return view('Admin/Kelulusan/Transkrip/index', $data); // tampilkan view dashboard
    }

    public function ajaxDataTablesTranskrip()
    {
        $kelas_data_dapodik = $this->request->getPost('kelas_data_dapodik');
        $data_siswaModel = new data_siswaModel();
        if ($kelas_data_dapodik == '') {
        ...
        return DataTable::of($builder)
            ...
            ->add('action', function ($row) {
                $nama_file = sprintf("transkrip_ijazah_%s_%s.pdf", preg_replace('/[^A-Za-z0-9]/', '_', $row->nama_lengkap_data_siswa), $row->kelas_data_dapodik);
                // hapus petik yang ada pada nama file
                $nama_file = str_replace("'", "", $nama_file);
                $nama_file = str_replace("__039", "", $nama_file);
                ...
                if (file_exists($file_path)) {
                    return '
                        <div class="flex align-items-center list-user-action">
                            <a class="btn btn-sm btn-icon btn-info" href="' . base_url('Assets/pdf/TranskripIjazah/' . $nama_file) . '" target="_blank" data-toggle="tooltip" data-placement="top" title="Lihat Rapor Pelengkap">
                                <svg class="icon-24" width="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                        d="M14.7379 2.76175H8.08493C6.00493 2.75375 4.29993 4.41175 4.25093 6.49075V17.2037C4.20493 19.3167 5.87993 21.0677 7.99293 21.1147C8.02393 21.1147 8.05393 21.1157 8.08493 21.1147H16.0739C18.1679 21.0297 19.8179 19.2997 19.8029 17.2037V8.03775L14.7379 2.76175Z"
                                        stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
    ...
    public function generateTranskripIjazah()
    {
        $nama_kelas = $this->request->getPost('kelas_data_dapodik');
        $siswaModel = new data_siswaModel(); // ambil model data siswa
        $urutanMapelModel = new urutanMapelModel();
        ...
        foreach ($data_nilai_group as $dtn) {
            ...
            $data['urutan_mapel_lokal'] = $urutanMapelLokal;
            $data['imageSrc'] = $imageSrc;
            $html = view('Admin/Kelulusan/Transkrip/Cetak/cetak_transkrip', $data);
    ...
    public function cetakSemuaTranskripIjazah()
    {
        $nama_kelas = $this->request->getPost('kelas_data_dapodik');
        $siswaModel = new data_siswaModel(); // ambil model data siswa
        $urutanMapelModel = new urutanMapelModel();
        ...
        if ($data_siswa) { // jika data siswa ada
            ...
            $data['urutan_mapel_pilihan'] = $urutanMapelPilihan;
            $data['urutan_mapel_lokal'] = $urutanMapelLokal;
            $html = view('Admin/Kelulusan/Transkrip/Cetak/cetak_all_transkrip', $data);
            $mpdf->WriteHTML($html);
            $nama_file = 'transkrip_ijazah_' . $nama_kelas . '.pdf';
            
            // jika sudah ada file dengan nama yang sama, maka file tersebut akan dihapus
            if (file_exists(FCPATH . 'Assets/pdf/TranskripIjazah/' . $nama_file)) {
                unlink(FCPATH . 'Assets/pdf/TranskripIjazah/' . $nama_file);
            }
            
            // view result
            $mpdf->Output(FCPATH . 'Assets/pdf/TranskripIjazah/' . $nama_file, 'F');
            return $this->response->setJSON([
                'error' => false,
                'status' => '200',
                'data' => $nama_file,

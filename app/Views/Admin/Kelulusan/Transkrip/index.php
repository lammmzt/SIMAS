...
function dataTablesDataSiswa() {
    $('#table_data_siswa').DataTable({
        ...
        responsive: true,
        ajax: {
            url: '<?= base_url('Kelulusan/ajaxDataTablesTranskrip') ?>',
            type: 'POST',
            data: function(data) {
...
$('#btn-generate').click(function() {
    ...
    if (kelas_data_dapodik == '') {
        ...
        $('#loading-page').show();
        $.ajax({
            url: '<?= base_url('Kelulusan/generateTranskripIjazah') ?>',
            type: 'POST',
            data: {
...
$('#btn-cetak-all').click(function() {
    ...
    if (kelas_data_dapodik == '') {
        ...
        $('#loading-page').show();
        $.ajax({
            url: '<?= base_url('Kelulusan/cetakSemuaTranskripIjazah') ?>',
            type: 'POST',
            data: {
                kelas_data_dapodik: kelas_data_dapodik
            },
            success: function(response) {
                if (response.error == false) {
                    $('#loading-page').hide();
                    window.open('<?= base_url('Assets/pdf/TranskripIjazah/') ?>' + response.data,
                        '_blank');
                } else {
                    $('#loading-page').hide();
                    sweetalert('error', 'Gagal', response.data);
